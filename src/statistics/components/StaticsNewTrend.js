...
const ProjectStaticsNewTrend = (props) => {
    const { statisticsStore } = props;
    const { findReport, statisticsNewWorkItemCount, findProjectList } = statisticsStore;
    ...
    const setStatisticsData = (params) => {
        ...
        statisticsNewWorkItemCount(params).then(res => {
            if (res.code === 0) {
                const list = res.data;
                let seriesValue = []
    ...
    return (
        <div className="statistics-work">
            <div className="statistics-work-top">
                <div className="statistics-work-title">事项新增趋势</div>
                <Form
                    name="form"
                    form={form}
...
export default withRouter(inject('statisticsStore')(observer(ProjectStaticsNewTrend)));