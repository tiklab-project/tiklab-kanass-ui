...
import sprintstatisticStore from "../store/SprintStatisticStore";
const { RangePicker } = DatePicker;


const SprintStaticsTotalNewTrend = (props) => {
    const { findProjectList } = statisticStore;
    const {statisticsSprintWorkItemTotalCountList} = sprintstatisticStore;
    ...
    const setStatisticsData = (params) => {
        ...
        statisticsSprintWorkItemTotalCountList(params).then(res => {
            if (res.code === 0) {
                const data = res.data;
                let seriesValue = []
                seriesValue.push({
...
export default withRouter(SprintStaticsTotalNewTrend);