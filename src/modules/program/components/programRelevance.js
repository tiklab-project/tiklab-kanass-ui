...
const ProgramRelevance = (props) => {
    const { programStore, programId } = props;
    const { findProjectIsOrNotRe, noRelatedProjects, relatedProjects, addRelevance, findProjectList } = programStore;
    ...
    return <div >
        <Button type="primary" onClick={() => addProgram()}>关联项目</Button>
        <Modal
            title={`关联项目集至：项目集1`}
            visible={visible}
            onOk={sumitFrom}
...
export default inject('programStore')(observer(ProgramRelevance));