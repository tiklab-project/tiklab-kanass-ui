...
const PlanTable = (props) => {
                                        ...
                                        <div className="name cell" style={{paddingLeft: `${deep * 20 + 10}`}}>
                                            {
                                                (childItem.children && childItem.children.length>0) || (childItem.workItemList && childItem.workItemList.length>0) ?
                                                    ...
                                                    <svg className="icon" aria-hidden="true" onClick={() => setOpenOrClose(childItem.id)}>
                                                        <use xlinkHref="#iconquanzhankai"></use>
                                                    </svg> :
                                                    <svg className="icon" aria-hidden="true" onClick={() => setOpenOrClose(childItem.id)}>
                                                        <use xlinkHref="#iconquanshouqi"></use>
                                                    </svg>
                                                    ) : ""
                                            }
                            ...
                            <li key={childItem.id} className={isExpandedTree(faid) ? null : 'plan-table-hidden'} >
                                ...
                                >
                                    <div className="plan-table-item-content">
                                        ...
                                        <div className="start-time cell">{childItem.startTime}</div>
                                        <div className="end-time cell">{childItem.endTime}</div>
                                        <div className="state cell">{childItem.workStatus.name}</div>
    ...
    return (
        <div className="plan-workitem">
            ...
            <Modal title="是否删除" visible={isWorkModalVisible} onOk={deletePlan} onCancel={()=>setIsWorkModalVisible(false)}>
                是否删除计划？
            </Modal>
            <Modal title="是否删除" visible={isPlanModalVisible} onOk={deletePlanWorkItem} onCancel={()=>setIsPlanModalVisible(false)}>
                是否删除事项？
            </Modal>
