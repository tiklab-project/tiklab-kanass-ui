...
import { orgaRouterLocal,orgaRouterAcc } from "./orgaRouter"
...
const OrgaAside=(props)=>  {
    ...
    const [router,setRouter] = useState(authType == "acc" ? orgaRouterAcc : orgaRouterLocal)
    // useEffect(() => {
    //     let data = pluginConfig("project-license").filter(item => item.routers);
    //     if(data.length > 0){
    //         let newRouter;
    //         data = data.map(item => {
    ...
    //         })
    //         setRouter(orgaRouter.concat(newRouter));
    //     }
...
export default withRouter(inject(PLUGIN_STORE)(observer(OrgaAside)))
