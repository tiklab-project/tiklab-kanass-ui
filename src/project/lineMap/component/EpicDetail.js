/*
 * @Descripttion: 史诗详情, 弃用
 * @version: 1.0.0
 * @Author: 袁婕轩
 * @Date: 2022-04-09 16:39:00
 * @LastEditors: 袁婕轩
 * @LastEditTime: 2024-12-27 13:35:23
 */
...
const EpicDetail = (props) => {
    ...
    const selectKeyFun = (epicId) => {
        props.history.push(`/project/${projectId}/epic/${epicId}`)
        setShowMenu(false)
    }
    ...
    const deEpic = () => {
        deleteEpic({id: epicId}).then(res => {
            if(res.code === 0){
                props.history.push(`/project/${projectId}/linemap`)
            }
        })
    }
                        ...
                        epicInfo && <Fragment>
                            <div className="epic-detail-top">
                                <div className="epic-breadcrumb">
                                    <span className="epic-breadcrumb-first" onClick={() => props.history.push(`/project/${projectId}/linemap`)}>需求集</span>
