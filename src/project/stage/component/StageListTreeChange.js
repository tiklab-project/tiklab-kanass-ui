 ...
 * @param {*} stage 
 */
const updateStageInfo = (tree, id, stage) => {
    for (let i = 0; i < tree.length; i++) {
        if (tree[i].id === id) {
            tree[i] = {...tree[i], ...stage};
            // 插入成功后返回true
            ...
            if (tree[i].children != null) {
                updateStageInfo(tree[i].children,id, stage)
            }
        }
    }
}
...
const updateTree = (tree, moveToId, id) => { // 通过id从数组（树结构）中移除元素
    ...
    insertNode(tree, moveToId, deleteNode)
...
const updateWorkTree = (tree, moveToId, id) => { // 通过id从数组（树结构）中移除元素
    const deleteNode = removeWorkItemForUpdate(tree, id)
    insertWorkItem(tree, moveToId, deleteNode)
    console.log(deleteNode)
    return tree;
}


const removeWorkItemForUpdate = (tree, id) => {
    // 遍历数组中的每个节点
    for (let i = 0; i < tree.length; i++) {
        const node = tree[i];
        ...
        // 递归调用，处理当前节点的子节点
        if (node.children) {
            const deletedNode = removeWorkItemForUpdate(node.children, id);
            if (deletedNode) {
                return deletedNode; // 如果子节点中找到了要删除的节点，则返回该节点
            }
        }
        if (node.childrenWorkItem) {
            const deletedNode = removeWorkItemForUpdate(node.childrenWorkItem, id);
            if (deletedNode) {
                return deletedNode; // 如果子节点中找到了要删除的节点，则返回该节点
            }
        }
...
const insertWorkItem = (tree, moveToId, node) => {
    if(!moveToId){
        tree.unshift(node);
    }else {
        for (let i = 0; i < tree.length; i++) {
            if (tree[i].id === moveToId) {
                if (tree[i].childrenWorkItem != null) {
                    tree[i].childrenWorkItem.unshift(node)
                } else {
                    tree[i].childrenWorkItem = []
                    tree[i].childrenWorkItem.unshift(node)
                }
                ...
                if (tree[i].children != null) {
                    if (insertWorkItem(tree[i].children, moveToId, node)) {
                        return true; // 如果在子节点中成功插入了新节点，则返回true
                    }
                }
                if (tree[i].childrenWorkItem != null) {
                    if (insertWorkItem(tree[i].childrenWorkItem, moveToId, node)) {
                        return true; // 如果在子节点中成功插入了新节点，则返回true
                    }
                }
            }
...
// 更新父级
export { appendNodeInTree, appendStageInTree,updateStageInfo, updateTree, removeNodeInTree, removeNodeInTreeAddChildren, updateWorkTree };