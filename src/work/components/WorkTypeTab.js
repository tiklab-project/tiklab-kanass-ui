    ...
    return (
        // <div className="workitem-filter">
            <div className="worktype-tabs">
                <div className={`tabs-bar ${tabValue.id === "all" ? "tabs-bar-select" : ""}`} onClick={() => selectType("all")} key = {"all"}>全部</div>
                {
                    workSystem && workSystem.map(item => {
                       return <div key = {item.id} className={`tabs-bar ${tabValue.id === item.workType.id ? "tabs-bar-select" : ""}`} onClick={() => selectType(item.workType)}>{item.workType.name}</div>
                    })
                }
                {
